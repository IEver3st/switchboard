...
const outputDirectory = join(projectRoot, 'design-qa', 'device-workbench-cleanup', 'after');
...
async function run() {
  ...
  await evaluate(window, 'window.switchboard.updateSettings({ uiScalePercent: 100, onboardingCompleted: true, developerMode: true })');
  ...
  await clickSelector(window, '.lighting-zones-trigger');
  await capture(window, '1420x900-zones.png');
  ...
  await setViewport(window, { width: 1080, height: 720 });
  await evaluate(window, 'window.switchboard.updateSettings({ uiScalePercent: 125, onboardingCompleted: true, developerMode: true })');
  await delay(150);
  await openG502(window);
  await waitFor(window, `document.querySelector('.mouse-stage canvas')?.dataset.renderState === 'ready'`);
  await waitFor(window, `document.querySelector('.mouse-stage canvas')?.dataset.renderState === 'ready'`);
  const scaled = await layoutMetrics(window);
  await capture(window, '1080x720-scale-125.png');
  assertLayout(scaled, '1080x720 at 125%');
  await capture(window, '1080x720-scale-125.png');
  report.captures.push({ filename: '1080x720-scale-125.png', metrics: scaled });
  await clickSelector(window, '[aria-label="Mouse lighting"]');
  await waitMouse(window, value => !value.capabilities.lighting.enabled, 'lighting off');
  await capture(window, '1080x720-lighting-off.png');
  await clickSelector(window, '[aria-label="Mouse lighting"]');
  await waitMouse(window, value => value.capabilities.lighting.enabled, 'lighting on');
  await evaluate(window, 'window.switchboard.updateSettings({ uiScalePercent: 100, onboardingCompleted: true, developerMode: true })');
  await setViewport(window, { width: 1080, height: 720 });
  await openG502(window);
  ...
  await delay(100);
  await evaluate(window, `document.querySelector('.lighting-zones-trigger').focus()`);
  window.webContents.sendInputEvent({ type: 'keyDown', keyCode: 'Enter' });
  window.webContents.sendInputEvent({ type: 'char', keyCode: '\r' });
  await delay(80);
  window.webContents.sendInputEvent({ type: 'keyUp', keyCode: 'Enter' });
  await waitFor(window, `Boolean(document.querySelector('.lighting-zones-popover'))`);
  await capture(window, '1080x720-keyboard-zones-reduced-motion.png');
  window.webContents.sendInputEvent({ type: 'keyDown', keyCode: 'Escape' });
  window.webContents.sendInputEvent({ type: 'keyUp', keyCode: 'Escape' });
  await waitFor(window, `!document.querySelector('.lighting-zones-popover')`);
  if (!await evaluate(window, `document.activeElement?.classList.contains('lighting-zones-trigger')`)) throw new Error('Zone editor did not restore keyboard focus.');
  report.interactions.push({ name: 'zone-keyboard-path', open: 'Enter', close: 'Escape', focusRestored: true, reducedMotion: true });
  const stressSnapshot = await evaluate(window, 'window.switchboard.getSnapshot()');
  const stressMouse = stressSnapshot.devices.find(device => device.kind === 'mouse');
  stressMouse.capabilities.lighting.zones = Array.from({ length: 26 }, (_, index) => ({ ...stressMouse.capabilities.lighting.zones[0], id: `zone-${index + 1}`, label: `Zone ${index + 1}` }));
  stressMouse.capabilities.dpi.stages = [800, 1200, 1600, 2400, 3200];
  window.webContents.send('system:snapshot-updated', stressSnapshot);
  await waitFor(window, `document.querySelector('.lighting-zones-trigger')?.textContent.includes('26 zones')`);
  await clickSelector(window, '.lighting-zones-trigger');
  await waitFor(window, `document.querySelectorAll('.lighting-zone').length === 26`);
  await capture(window, '1080x720-26-zones-fixture.png');
  const zoneBounds = await evaluate(window, `(() => { const r = document.querySelector('.lighting-zones-popover').getBoundingClientRect(); return { top: r.top, bottom: r.bottom, left: r.left, right: r.right, height: innerHeight, width: innerWidth }; })()`);
  if (zoneBounds.top < 0 || zoneBounds.bottom > zoneBounds.height || zoneBounds.left < 0 || zoneBounds.right > zoneBounds.width) throw new Error('26-zone editor exceeds viewport.');
  report.interactions.push({ name: '26-zone-layout-fixture', bounds: zoneBounds });
  await window.webContents.debugger.detach();
  await evaluate(window, `document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }))`);
  stressMouse.connected = false;
  stressMouse.capabilities.dpi.writable = false;
  stressMouse.capabilities.reportRate.writable = false;
  stressMouse.capabilities.buttonAssignments.writable = false;
  stressMouse.capabilities.lighting.writable = false;
  stressMouse.capabilities.onboardMemory.writable = false;
  stressMouse.capabilities.lighting.stateReason = 'Mouse disconnected. Reconnect the device to change lighting.';
  window.webContents.send('system:snapshot-updated', stressSnapshot);
  await waitFor(window, `document.querySelector('#active-mouse-dpi')?.disabled === true`);
  await capture(window, '1080x720-disconnected-fixture.png');
  assertLayout(await layoutMetrics(window), 'disconnected');
...
function assertLayout(metrics, label) {
  if (metrics.viewportScrollHeight > metrics.viewportHeight + 1) throw new Error(`${label} has vertical scrolling: ${JSON.stringify(metrics)}`);
...
async function waitFor(window, expression, timeout = 10_000) {
  ...
  console.log('failure state', await evaluate(window, `document.body.innerText.slice(0, 1400)`));
  await capture(window, 'failure-state.png');
