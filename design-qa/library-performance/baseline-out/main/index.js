import { BrowserWindow, Menu, Tray, app, clipboard, desktopCapturer, dialog, globalShortcut, ipcMain, nativeImage, net, protocol, screen, session, shell } from "electron";
...
var setCaptureConfigInputSchema = captureConfigSchema.omit({ clipsDirectory: true }).partial().extend({ defaultTrackLevels: defaultClipTrackLevelsSchema.partial().optional() });
...
var clipCanvasSizeSchema = z.enum(["original", "9:16"]);
...
var feedbackReportKindSchema = z.enum(["bug", "feature"]);
var feedbackReportInputSchema = z.object({
...
function getEncodingPreset(config) {
	...
	const targetMbps = BASE_60_FPS_MBPS[resolution][qualityIndex] * CODEC_FACTOR[config.codec] * FPS_FACTOR[config.fps];
...
var AudioEndpointDiscovery = class {
	...
	async list() {
		...
		const stdout = await run$1(command, commandArguments, cwd, environment, this.options.timeoutMs ?? 15e3);
...
function run$1(command, commandArguments, cwd, environment, timeoutMs) {
...
var AppUpdateService = class {
	...
	installDownloadedUpdate() {
		...
		if (!this.updater || this.state.status !== "downloaded") throw new Error("No downloaded Switchboard update is ready to install.");
		this.publish({
			status: "installing",
			error: null
		});
...
var LogitechRgbEffectsController = class LogitechRgbEffectsController {
	...
	buildCapability(writable) {
		...
		return {
			...
			stateReason: this.acknowledged ? "The mouse acknowledged the live lighting command; this HID++ effect path has no state readback." : "The mouse reports available effects and zones, but not the currently visible live effect. Choose a setting to take control.",
			physicalEffectVerified: false,
			profileMode: "software",
			source: "software",
			unavailableReason
...
var G502NativeSession = class G502NativeSession {
	...
	unsubscribe;
	closed = false;
	constructor(transport, deviceIndex, dpiFeatureIndex, buttonSpyFeatureIndex, batteryFeatureIndex, reportRateFeatureIndex, rgbEffectsFeatureIndex, onboardProfilesFeatureIndex, rgbLighting, supportedDpi, currentDpi, preferredShiftDpi, preferredStages, onboard) {
		this.transport = transport;
		this.deviceIndex = deviceIndex;
		...
		this.rgbEffectsFeatureIndex = rgbEffectsFeatureIndex;
	...
	static async open(endpoint, previous) {
		...
		try {
			...
			return new G502NativeSession(transport, deviceIndex, dpiFeatureIndex, buttonSpyFeatureIndex, batteryFeatureIndex, reportRateFeatureIndex, rgbEffectsFeatureIndex, onboardProfilesFeatureIndex, rgbLighting, supportedDpi, currentDpi, previous?.dpi?.shiftDpi, previous?.dpi?.stages, onboard);
		} catch (error) {
	...
	async getCapabilities() {
		...
		if (this.onboard) {
			...
			capabilities.lighting = onboardWritable || !this.rgbLighting ? this.onboard.profile.buildLighting(this.onboard.mode, onboardWritable) : this.rgbLighting.buildCapability(true);
		...
		if (!capabilities.lighting && this.rgbLighting) capabilities.lighting = this.rgbLighting.buildCapability(true);
	...
	async setControl(change) {
		...
		if (change.type === "lighting-enabled") {
			...
			this.assertOnboardWritable();
			await applyG502LightingEnabled(this.transport, this.deviceIndex, this.rgbEffectsFeatureIndex, change.enabled, () => this.mutateProfile((profile) => profile.setLightingEnabled(change.enabled)));
			return;
		}
		if (change.type === "lighting-color") {
			if (this.usesLiveLighting) {
	...
	assertOnboardWritable() {
		if (!this.onboard || this.onboard.mode !== "onboard") throw new Error("Enable onboard memory before changing stored buttons or lighting.");
	}
	get usesLiveLighting() {
		return this.onboard?.mode !== "onboard" && this.rgbLighting !== null;
	}
	assertLiveLighting() {
		if (!this.usesLiveLighting) throw new Error("Turn off onboard memory to use live LIGHTSYNC effects and zone colors.");
	}
	assertStages(values) {
...
*/
async function applyG502LightingEnabled(transport, deviceIndex, rgbEffectsFeatureIndex, enabled, persist) {
	if (rgbEffectsFeatureIndex !== null) try {
...
async function setRgbLightingPower(transport, deviceIndex, featureIndex, enabled) {
	...
	try {
		...
		if ((await transport.request(deviceIndex, featureIndex, 8, [
			0,
			0,
			0
		]))[5] !== requestedMode) throw new Error("The mouse did not acknowledge the requested RGB power mode.");
	} catch (error) {
...
var RgbOwnershipReleaseError = class extends Error {
	constructor(cause) {
...
var directSessionRetryDelayMs = 3e4;
...
var LogitechDeviceModule = class {
	...
	async createG502XPlusFallback(transport, directEndpoint, context) {
		...
		let capabilities = {};
		if (directEndpoint?.path) capabilities = await this.readDirectCapabilities(directEndpoint, previous, id);
		else {
	...
	async readDirectCapabilities(endpoint, previous, deviceId) {
		...
		try {
			const capabilities = await (await this.ensureDirectSession(endpoint, previous)).getCapabilities();
...
var DeviceRegistry = class {
	...
	async discover() {
		...
		const hidDevices = activeModules.length > 0 ? await this.enumerateHidDevices() : [];
		if (this.disposed) return;
...
var switchboardIssueUrl = "https://github.com/IEver3st/switchboard/issues/new";
function buildFeedbackReportMarkdown(input, environment) {
	const isBug = input.kind === "bug";
	const sections = [`## ${isBug ? "Bug description" : "Requested capability"}`, input.description.trim()];
	const supportingDetails = input.supportingDetails?.trim();
	if (supportingDetails) sections.push(`## ${isBug ? "Steps to reproduce" : "Use case"}`, supportingDetails);
...
function buildFeedbackIssueUrl(input, environment) {
...
function buildFeedbackClipboardText(input, environment) {
	return `# [${input.kind === "bug" ? "Bug" : "Feature"}] ${input.title.trim()}\n\n${buildFeedbackReportMarkdown(input, environment)}`;
}
//#endregion
//#region src/main/services/feedback-handoff.ts
async function performFeedbackHandoff(input, environment, operations) {
	let copied = true;
	try {
...
var StateStore = class {
	...
	persist() {
		...
		this.persistChain = this.persistChain.catch(() => void 0).then(async () => {
			...
			if (this.persistedPayload !== null) await writeDurableState(`${this.filePath}.bak`, this.persistedPayload);
			await writeDurableState(this.filePath, payload);
	...
	async sample(forcePublish) {
		...
		try {
			...
			if (Boolean(this.options.recordSample) && (forcePublish || guardChanged || measuredAt - this.lastResourceRecordedAt >= recordInterval)) {
				...
				}) && this.options.getRendererRuntime) try {
...
var AutoCaptureEngine = class {
	...
	publish(forcedState) {
		...
		const latest = [...this.pending.values()].at(-1);
		const state = forcedState ?? (!automationEnabled || this.disposed ? "disabled" : this.saving > 0 ? "saving" : latest ? "pending" : this.listening ? "listening" : "idle");
		this.runtime = {
			...this.runtime,
			state,
...
var WarThunderTelemetryParser = class {
	...
	parse(payload, playerName, receivedAt = Date.now()) {
		const parsed = warThunderHudResponseSchema.parse(payload);
...
function eventFromMessage(message, playerName, timestamp) {
	...
	if (combat) {
		...
		if (combatantMatches(combat.target, playerName)) return createEvent(message.stream, message.id, "death", timestamp, "Vehicle lost", {
			sequence: message.id,
			code: combat.action.replace(" ", "_")
		});
		return null;
	...
	if (crashActor && combatantMatches(crashActor, playerName)) return createEvent(message.stream, message.id, "death", timestamp, "Vehicle lost", {
		sequence: message.id,
		code: "crashed"
	});
	return null;
...
function combatantMatches(value, playerName) {
...
var WarThunderProvider = class {
	...
	configure(settings) {
		...
		if (!this.playerName && this.status.state !== "stopped" && this.status.state !== "starting") this.status = {
			state: "degraded",
			message: "Enter your War Thunder nickname to identify personal events."
		};
		else if (this.playerName && wasMissingPlayerName && this.initialized && this.status.state === "degraded") this.status = {
			state: "listening",
	...
	async poll(lifecycle, baseline) {
		...
		try {
			...
			if (baseline || !this.initialized) {
				...
				this.initialized = true;
			} else for (const event of this.parser.parse(payload, this.playerName)) {
				this.eventsEmitted += 1;
				this.status = {
					state: "listening",
					lastEventAt: event.timestamp
...
var AppController = class {
	...
	refreshAudioDevices(force = false) {
		...
		const refresh = this.audioEndpointDiscovery.list().then((devices) => {
	...
	duplicateAudioPreset(input) {
		return this.store.update((draft) => {
			...
			const copy = snapshotAudioPathPreset(draft.audio, source.kind, `user-${source.kind}-${randomUUID()}`, `${source.name} copy`);
	...
	async setAudioMonitoring(input) {
		const before = this.store.get();
		if (before.audio.capabilities.monitoring === "unavailable") throw new Error("Low-latency microphone monitoring is unavailable until Audio.Host owns the microphone stream.");
	...
	async updateSettings(input) {
		...
		if (typeof input.automaticAppUpdates === "boolean" || typeof input.automaticAppUpdateDownloads === "boolean" || typeof input.installAppUpdatesOnNextStartup === "boolean") this.appUpdates.setPreferences(appUpdatePreferences(snapshot.settings));
	...
	async handoffFeedbackReport(input) {
		return performFeedbackHandoff(input, {
	...
	async resetSettings(scope) {
		...
		let snapshot = this.store.update((draft) => {
			...
			if (scope === "devices") draft.settings.deviceAppearanceOverrides = {};
			if (scope === "audio") {
				draft.audio = createResetAudioState(draft.audio);
				const module = draft.modules.find((candidate) => candidate.id === "capability.audio-router");
				if (module) module.enabled = false;
	...
	async configureAudioEngine(snapshot) {
		if (!snapshot.audio.enabled) return snapshot;
		const hostSnapshot = audioHostSnapshotSchema.parse(await this.engines.request("audio", "configure", snapshot.audio, 3e4));
		this.applyAudioHostSnapshot(hostSnapshot);
		return this.store.get();
	}
	...
	toHostSettings(config, paths = this.capturePaths) {
		...
		return {
			...
			audioFallbackReason: requestedSwitchboardAudioReady || !config.includeSystemAudio && !config.includeChatAudio && !processedMicrophoneRequested ? null : "Switchboard audio routing is unavailable for one or more replay inputs; Windows default devices are being used where needed."
		};
	}
	...
	scheduleAudioHostRecovery(reason) {
		...
		this.audioRestartTimer = setTimeout(() => {
			...
			this.startAudioEngine().catch((restartError) => {
				this.store.update((draft) => {
...
function assertTrustedSender$1(event, getMainWindow) {
	const window = getMainWindow();
	if (!window || window.isDestroyed() || event.sender.id !== window.webContents.id) throw new Error("Rejected IPC from an untrusted webContents instance.");
...
function registerIpc(controller, getMainWindow) {
	...
	handle(ipcChannels.handoffFeedbackReport, getMainWindow, (input) => feedbackReportInputSchema.parse(input), (input) => controller.handoffFeedbackReport(input));
...
async function renderMontageV2(input) {
	...
	try {
		...
		const durationSeconds = input.project.durationMs / 1e3;
		const budgetKbps = exportPresetBytes[input.preset] * 8 * .94 / Math.max(.1, durationSeconds) / 1e3;
		const audioKbps = budgetKbps >= 620 ? 128 : budgetKbps >= 420 ? 96 : 64;
		const videoKbps = Math.floor(Math.max(120, budgetKbps - audioKbps));
		if (budgetKbps < audioKbps + 120) throw new Error("This montage is too long for the selected file size. Choose a larger preset or shorten the sequence.");
		const passLog = join(temporaryDirectory, "montage-pass");
		await run(executable, [
			"-hide_banner",
			"-loglevel",
...
async function renderMontageSegment(executable, entry, destination, target, signal) {
...
function buildMontageV2VideoFilter(segment, target) {
	return `[0:v:0]trim=start=${(segment.trimStartMs / 1e3).toFixed(3)}:end=${(segment.trimEndMs / 1e3).toFixed(3)},setpts=PTS-STARTPTS,${target.canvasSize === "9:16" ? `crop='if(gte(iw/ih,0.5625),trunc(ih*0.5625/2)*2,iw)':'if(gte(iw/ih,0.5625),ih,trunc(iw/0.5625/2)*2)',scale=${target.width}:${target.height}:flags=lanczos` : `scale=${target.width}:${target.height}:force_original_aspect_ratio=decrease:flags=lanczos,pad=${target.width}:${target.height}:(ow-iw)/2:(oh-ih)/2:color=black`},setsar=1,fps=${target.fps.toFixed(3)},format=yuv420p[vout]`;
}
...
async function probeMontageAudio(path) {
	const output = await run(findExecutable("SWITCHBOARD_FFPROBE", "ffprobe"), [
		"-v",
		"error",
		"-print_format",
		"json",
...
async function getAudioStreamCount(path) {
	const output = await run(findExecutable("SWITCHBOARD_FFPROBE", "ffprobe"), [
		"-v",
		"error",
		"-select_streams",
...
function run(executable, arguments_, signal) {
	return new Promise((resolvePromise, reject) => {
		if (signal?.aborted) {
			reject(abortError());
			return;
...
var MontageV2Service = class {
	...
	async saveDraft(input) {
		...
		this.manifest.drafts = this.manifest.drafts.sort((left, right) => right.updatedAt - left.updatedAt).slice(0, maximumDrafts);
		await this.persist();
	...
	async export(input, clips) {
		...
		const finalBytes = input.preset === "original" ? proportionalSourceBytes : presetTargetBytes(input.preset);
