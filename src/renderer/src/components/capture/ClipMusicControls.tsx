...
export function ClipMusicControls({ music, durationMs, onChange }: {
  ...
  return <section className="editor-clip-music">
    ...
    {music ? <>
      ...
      <label className="editor-adjustment"><span>Music volume<output>{Math.round(music.volume * 100)}%</output></span><Slider variant="fader" aria-label="Clip music volume" min={0} max={100} step={1} value={[music.volume * 100]} onValueChange={([value]) => { if (value !== undefined) change({ volume: value / 100 }); }} /></label>
