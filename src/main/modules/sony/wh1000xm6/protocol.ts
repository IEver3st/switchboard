...
export const xm6InitialQueries = [
  [0x00, 0x00], [0x06, 0x00], [0x22, 0x00], [0x66, 0x19], [0x56, 0x00],
...
export type Xm6Event =
  ...
  | { type: 'noise-control'; mode: SonyNoiseControlMode; ambientLevel: number; focusOnVoice: boolean }
  | { type: 'equalizer'; presetId: string; gainsDb: number[] }
...
export function xm6NoiseControl(mode: SonyNoiseControlMode, ambientLevel: number, focusOnVoice: boolean): Uint8Array {
  const level = Math.max(1, Math.min(20, Math.round(ambientLevel)));
  if (mode === 'off') return Uint8Array.from([0x68, 0x19, 1, 0, 0, 0, level, 0, 0]);
  return Uint8Array.from([0x68, 0x19, 1, 1, mode === 'ambient' ? 1 : 0, mode === 'ambient' && focusOnVoice ? 1 : 0, level, 0, 0]);
}

export function xm6EqualizerPreset(presetId: string): Uint8Array[] {
  const preset = xm6EqualizerPresets.find(([id]) => id === presetId);
  if (!preset) throw new Error('Unsupported WH-1000XM6 equalizer preset.');
  return [Uint8Array.from([0x58, 0x00, preset[1], 0x00]), Uint8Array.from([0x56, 0x00])];
}

export function xm6EqualizerBands(gainsDb: number[]): Uint8Array {
  if (gainsDb.length !== 10) throw new Error('WH-1000XM6 equalizer requires ten bands.');
  return Uint8Array.from([0x58, 0x00, 0xa0, 10, ...gainsDb.map((gain) => Math.max(0, Math.min(12, Math.round(gain) + 6)))]);
}
...
export function parseXm6Event(frame: SonyFrame): Xm6Event | null {
  ...
  if ((command === '67:19' || command === '69:19') && p.length >= 8) {
    ...
    return { type: 'noise-control', mode, focusOnVoice: p[offset + 2] === 1, ambientLevel: Math.max(1, Math.min(20, p[offset + 3]!)) };
  }
  if ((command === '57:0' || command === '59:0' || command === '59:4') && p.length >= 4) {
    const count = p[3]!;
    ...
    return { type: 'equalizer', presetId: preset?.[0] ?? `unknown-${p[2]!.toString(16)}`, gainsDb };
  }
